...
@dataclass
class MomentumDist:
    p0: float = 0.0          # mean momentum ⟨p⟩/(ℏ k_L) - COM at zero (standard)
                             # Use 0.1 for non-zero center-of-mass momentum
    sigma_p: float = 0.05    # width σ_p = 0.05 ℏk_L ✓
...
class DBDPhysics:
    ...
    - Initial momentum: p0 = 0.0 (COM at zero) - STANDARD
      * Use p0 = 0.1 if you want non-zero center-of-mass momentum
    ...
    def __init__(self, 
        ...
        # p0 = 0.0 (COM at zero) is standard for symmetric beam-splitter
        # Change to p0 = 0.1 if you need non-zero COM momentum
        if mdist is None:
            self.mdist = MomentumDist(p0=0.0, sigma_p=0.05)  # σp = 0.05 ℏkL ✓
        else:
            self.mdist = mdist
