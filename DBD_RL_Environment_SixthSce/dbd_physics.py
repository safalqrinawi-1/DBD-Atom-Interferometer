...
class DBDPhysics:
    ...
    def __init__(self, 
        ...
        if mdist is None:
            self.mdist = MomentumDist(p0=0.0, sigma_p=0.05)
        else:
            self.mdist = mdist
    ...
    def get_populations(self) -> np.ndarray:
        ...
        for i, wgt in enumerate(self.weights):
            ...
            populations[0] += wgt * abs(c_p) ** 2
            populations[1] += wgt * abs(c_plus2) ** 2
            populations[2] += wgt * abs(c_minus2) ** 2
            populations[3] += wgt * abs(c_plus4) ** 2
            populations[4] += wgt * abs(c_minus4) ** 2
