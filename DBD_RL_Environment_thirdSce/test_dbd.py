...
def test_simple(episodes=5):
    """Simple console testing without visualization."""
    print(f"\n[TEST] DBD Environment (Simple Mode - 30 Steps per Episode)")
    print("=" * 60)
    ...
    for episode in range(episodes):
        ...
        for step in range(env.steps_per_episode):
            ...
            beam_score = info.get('beam_splitting_score', 0.0)
            print(
                f"Step {step+1:2d} | Action: {action[0]:.4f} | Reward: {reward:.4f} | "
            ...
            if step > 0 and step % 5 == 0:
                print(
                    f"    Progress: Step {step+1}/{env.steps_per_episode} | Beam Score: {beam_score:.4f}"
                )
...
def test_with_visualization(episodes=5):
    ...
    print(f"\n[TEST] DBD Environment (Visualization Mode - 30 Steps per Episode)")
    print("=" * 70)
    ...
    class SimpleVisualizer:
        ...
        def update(self, populations, reward, step_count, steps_per_episode):
            ...
            title = self.font_large.render(
                "DBD Environment - Detuning Optimization", True, self.WHITE
            )
            ...
            progress_text = self.font_medium.render(
                f"Progress: {progress_pct:.1f}%", True, self.WHITE
            )
            self.screen.blit(progress_text, (20, 60))

            # Step and reward info
            step_text = self.font_medium.render(
                f"Step: {step_count}/{steps_per_episode}", True, self.GREEN
            )
            reward_text = self.font_medium.render(
                f"Reward: {reward:.4f}", True, self.YELLOW
            )
            self.screen.blit(step_text, (20, 90))
            self.screen.blit(reward_text, (250, 90))

            # Population header
            pop_header = self.font_medium.render(
                "Quantum State Populations:", True, self.BLUE
            )
            ...
            for i, (name, pop, color) in enumerate(
                zip(state_names, populations, colors)
            ):
                # State name and percentage
                state_text = self.font_small.render(
                    f"{name}: {pop:.1f}%", True, color
                )
                ...
                pygame.draw.rect(
                    self.screen, self.GRAY, (bar_x, y_pos, bar_width, bar_height), 2
                )  # Outline
                pygame.draw.rect(
                    self.screen, color, (bar_x, y_pos, bar_fill, bar_height)
                )  # Fill

                # Percentage on bar
                if pop > 0.1:  # Only show if visible
                    pct_text = self.font_small.render(
                        f"{pop*100:.1f}%", True, self.BLACK
                    )
                    self.screen.blit(pct_text, (bar_x + bar_fill - 30, y_pos + 5))

                y_pos += 35

            # Beam splitting score (recomputed for display)
            total_pm2 = float(populations[1] + populations[2] + 1e-8)
            balance = 1.0 - float(abs(populations[1] - populations[2])) / total_pm2
            symmetric_pop = balance * min(1.0, total_pm2)
            efficiency_y = y_pos + 20
            efficiency_text = self.font_medium.render(
                f"Beam Split Score: {symmetric_pop:.4f}", True, self.YELLOW
            )
            ...
            pygame.draw.rect(
                self.screen, self.GRAY, (20, progress_y, progress_width, 20), 2
            )
            pygame.draw.rect(
                self.screen, self.BLUE, (20, progress_y, progress_fill, 20)
            )

            # Instructions
            instructions = self.font_small.render(
                "Press ESC or close window to exit", True, self.WHITE
            )
    ...
    try:
        for episode in range(episodes):
            ...
            for step in range(env.steps_per_episode):
                ...
                if step > 0 and step % 5 == 0:
                    print(
                        f"    Progress: Step {step+1}/{env.steps_per_episode} | Symmetric Pop: {symmetric_pop:.4f}"
                    )
...
def main():
    ...
    while True:
        try:
            episodes_input = input(
                "Enter number of episodes to test (default 5): "
            ).strip()
            if episodes_input == "":
                episodes = 5
            else:
    ...
    if test_mode == "simple":
        ...
        test_with_visualization(episodes)
