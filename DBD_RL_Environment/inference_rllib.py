...
Examples:
  ...
  python inference_rllib.py --checkpoint "D:/path/to/checkpoint_000050/learner_group/learner/rl_module/default_policy"
"""
...
# Set this to your absolute checkpoint path to run by simply hitting "Run" on this script.
# Example: DEFAULT_CHECKPOINT = r"D:/MachineLearning_Projects/.../checkpoint_000000"
DEFAULT_CHECKPOINT: str = r"C:/Users/jbs/ray_results/DBD_PPO_20251012_221920/PPO_DBDEnv-v0_5a84d_00000_0_2025-10-12_22-19-20/checkpoint_000019"


def resolve_module_dir(checkpoint_path: Path) -> Path:
    """Resolve the RLModule directory (default_policy) from a checkpoint path.

    Accepts either the top-level checkpoint dir or the direct rl_module/default_policy dir.
    """
...
def run_inference(module_dir: Path, render: bool = True) -> dict:
...
def run_inference_from_checkpoint(checkpoint: str, render: bool = True) -> dict:
    ...
    return run_inference(module_dir, render=render)
...
def main():
    ...
    parser.add_argument(
        "--render",
        action="store_true",
        help="Print ANSI render each step.",
    )
    ...
    results = run_inference(module_dir, render=args.render)
    print(f"Episode completed! Final return: {results['episode_return']:.4f}")
    print(f"Final DBD efficiency: {results['final_symmetric_population']:.4f} ({results['final_symmetric_population']*100:.1f}%)")


def run_dbd_inference_simple(checkpoint_path: str = None) -> dict:
    """Simple function to run DBD inference. Use DEFAULT_CHECKPOINT if no path provided."""
    if checkpoint_path is None:
        if not DEFAULT_CHECKPOINT:
            raise ValueError("No checkpoint path provided and DEFAULT_CHECKPOINT is empty")
        checkpoint_path = DEFAULT_CHECKPOINT

    return run_inference_from_checkpoint(checkpoint_path, render=True)
