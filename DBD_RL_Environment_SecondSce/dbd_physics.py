...
def _clip_eps_pol(x: float) -> float:
    """Clamp polarization error to the typical range [0.0, 0.3]."""
    return float(np.clip(x, 0.0, 0.3))
...
class DBDParams:
    ...
    # Typically the polarization error lies in [0, 0.3]
    eps_pol: float = field(default=0.15, metadata={"range": "[0, 0.3]"})

    def __post_init__(self):
        # Enforce the typical range
        self.eps_pol = _clip_eps_pol(self.eps_pol)
...
class DBDPhysics:
    def __init__(self, pulse_duration=1.0, dt=1e-3,
        ...
        # Clamp polarization_error to the typical range before use
        pol = _clip_eps_pol(polarization_error)
        ...
        # If params was supplied, still enforce range for safety
        self.pars.eps_pol = _clip_eps_pol(self.pars.eps_pol)

        # Expose ω_rec for RL environment
    ...
    def reset(self, polarization_error=None):
        """Reset to initial state; optionally set ε_pol in [0, 0.3]."""
        if polarization_error is not None:
            self.pars.eps_pol = _clip_eps_pol(float(polarization_error))
        self.state[:] = 0.0
        self.state[0] = 1.0  # Reset to |p⟩
        self.current_time = 0.0
        self.pars.t_center = 0.5 * self.pulse_duration  # Re-center pulse
    ...
    def evolve_system(self, detuning_function, step_duration, p=None):
        ...
        # Return dict compatible with RL environment
        return {
            'success': True,
    ...
    def get_populations(self) -> np.ndarray:
        ...
        Returns:
            [P(|p⟩), P(|p+2ℏk⟩), P(|p-2ℏk⟩), P(|p+4ℏk⟩), P(|p-4ℏk⟩)]
        """
    ...
    def get_symmetric_population(self) -> float:
        """Population in |1,+⟩ (±2ℏk symmetric state)."""
        return float(abs(self.state[1]) ** 2)

    def get_beamsplitter_efficiency(self) -> float:
        """Beam-splitter efficiency: P(|p+2ℏk⟩) + P(|p-2ℏk⟩)."""
        bare_pops = self._bare_momentum_populations()
